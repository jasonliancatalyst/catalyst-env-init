...
if [[ ! -z $1 ]]; then
    ...
    echo "docker exec -i $webcont /siteroot/vendor/bin/phpunit ${1} ${2}> $(pwd)/phpunit_${1}_`date +%d-%m-%Y_%H_%M_%S`.log"
    docker exec -i $webcont bash -c "vendor/bin/phpunit \"${1}\" ${2}" > $(pwd)/phpunit_${1}_`date +%d-%m-%Y_%H_%M_%S`.log
    exit
else
    ...
    echo "docker exec -i $webcont vendor/bin/phpunit > $(pwd)/phpunit_`date +%d-%m-%Y_%H_%M_%S`.log"
    docker exec -i $webcont vendor/bin/phpunit > $(pwd)/phpunit_`date +%d-%m-%Y_%H_%M_%S`.log
    exit
fi
